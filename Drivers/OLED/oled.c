...
void OLED_WR_Byte(uint8_t dat, uint8_t cmd)
{
	...
	if (cmd)
		...
		DATA_COMMAND_clr;
	SSD1306_CS_clr;
	HAL_SPI_Transmit_DMA(&OLED_port, &dat, 1);
	SSD1306_CS_set;
	DATA_COMMAND_set;
}
...
void OLED_Display_block(uint8_t x, uint8_t y, uint8_t blockdata)
{
	uint8_t i;
	OLED_address(x, y);
	OLED_WR_Byte(blockdata, OLED_DATA);
}

//任意位置显示指定大小点阵
void OLED_Display_xy(uint8_t pos_x, uint8_t pos_y, uint8_t size_x, uint8_t size_y, uint8_t *dp)
{
	for (uint8_t image_y = 0; image_y < (size_y % 8 == 0 ? size_y / 8 : size_y / 8 + 1); image_y++)
	{
		OLED_address(pos_x, pos_y + image_y);
...
{
	HAL_SPI_Transmit(&OLED_port, &dat, 1, 0xff);
}
...
{
	uint8_t read = 0;
	HAL_SPI_Receive_DMA(&OLED_port, &read, 1);
	return read;
}

void Get_data_from_ROM_point(uint8_t *data)
{
	HAL_SPI_Receive_DMA(&OLED_port, data, 1);
}
...
void OLED_ShowNum(uint8_t x, uint8_t y, float num1, uint8_t len)
{
	...
	OLED_Display_GB2312_string(x - 24, y, ".",0); //显示小数点
	for (i = 0; i < len; i++)
	{
		t = num % 10;	//取个位数的数值
		num = num / 10; //将整数右移一位
		...
		{
		case 0:
			OLED_Display_GB2312_string(x, y, "0",0);
			break;
		case 1:
			OLED_Display_GB2312_string(x, y, "1",0);
			break;
		case 2:
			OLED_Display_GB2312_string(x, y, "2",0);
			break;
		case 3:
			OLED_Display_GB2312_string(x, y, "3",0);
			break;
		case 4:
			OLED_Display_GB2312_string(x, y, "4",0);
			break;
		case 5:
			OLED_Display_GB2312_string(x, y, "5",0);
			break;
		case 6:
			OLED_Display_GB2312_string(x, y, "6",0);
			break;
		case 7:
			OLED_Display_GB2312_string(x, y, "7",0);
			break;
		case 8:
			OLED_Display_GB2312_string(x, y, "8",0);
			break;
		case 9:
			OLED_Display_GB2312_string(x, y, "9",0);
			break;
		}
	}
}
...
void OLED_Init(void)
{
	...
	OLED_Clear();
	OLED_Display_GB2312_string(0, 0, "",0);
}
